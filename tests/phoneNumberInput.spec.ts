...
test.describe('phone input', () => {
  ...
  test('phone click s', async ({ page }) => {
    await page.locator(`data-test=Phone >> nth=0`).click();
    await page.waitForTimeout(250);
    await expect(page).toHaveScreenshot({ fullPage: true, scale: 'css' });
  });
  test('phone input number s', async ({ page }) => {
    await page.locator(`data-test-id=Phone >> nth=0`).click();
    await page.waitForTimeout(250);
    await page.locator(`data-test-id=Phone >> nth=0`).fill('+79875565432');
    await expect(page).toHaveScreenshot({ fullPage: true, scale: 'css' });
  });
  ...
  test('phone click m', async ({ page }) => {
    await page.locator(`data-test-id=Phone >> nth=2`).click();
    await page.waitForTimeout(250);
    await expect(page).toHaveScreenshot({ fullPage: true, scale: 'css' });
  });
  test('phone click l', async ({ page }) => {
    await page.locator(`data-test-id=Phone >> nth=4`).click();
    await page.waitForTimeout(250);
    await expect(page).toHaveScreenshot({ fullPage: true, scale: 'css' });
  });
